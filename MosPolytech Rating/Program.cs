...
namespace MosPolytech_Rating
{
    class Program
    {
        public static TelegramBotClient bot = new TelegramBotClient("TELEGRAM_BOT_TOKEN_HERE");
        Regex snilsRegex = new Regex(@"\d{3}-\d{3}-\d{3} \d{2}");
        Regex codeRegex = new Regex(@"\d{5}");
        ...
        private void Bot_OnMessage(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            try
            {
                if (e.Message.Text == "/start")
                {
                    ...
                    bot.SendTextMessageAsync(e.Message.Chat.Id, "Продолжая работу с этим ботом вы даете согласие на обработку персональных данных (а именно СНИЛСа). Приятного пользования\nv.0.4.68\nВведите свой СНИЛС с командой /snils (Пример: /snils 123-123-123 90 или /snils 11111, если нет СНИЛС)", replyMarkup: new ReplyKeyboardRemove());
                }
                else if (e.Message.Text.Contains("/snils"))
                {
                    ...
                    {
                        if (e.Message.Text.Split(' ').Length != 3 && e.Message.Text.Split(' ').Length != 2)
                        {
                            bot.SendTextMessageAsync(e.Message.Chat.Id, "Команда /snils использована неверно", replyMarkup: new ReplyKeyboardRemove());
                            return;
                        }
                        if (codeRegex.IsMatch(e.Message.Text.Split(' ')[1])) user.SNILS = e.Message.Text.Split(' ')[1];
                        else if (snilsRegex.IsMatch(e.Message.Text.Split(' ')[1] + " " + e.Message.Text.Split(' ')[2]))
                        {
                            user.SNILS = e.Message.Text.Split(' ')[1] + " " + e.Message.Text.Split(' ')[2];
                        }
                        else { bot.SendTextMessageAsync(e.Message.Chat.Id, "СНИЛС введен неверно", replyMarkup: new ReplyKeyboardRemove()); return; }
                ...
                else if (e.Message.Text.Contains("/loc"))
                {
                    ...
                    else
                    {
                        ...
                        markup.Keyboard = new KeyboardButton[][]
                        {
                            new KeyboardButton[]
                            {
                                new KeyboardButton("/fac 01.03.02 Прикладная математика и информатика")
                            },
                            new KeyboardButton[]
                            {
                            ...
                            new KeyboardButton[]
                            {
                                new KeyboardButton("/fac 09.03.03.01 Корпоративные информационные системы")
                            },
                            new KeyboardButton[]
                            {
                            ...
                            new KeyboardButton[]
                            {
                                new KeyboardButton("/fac 10.03.01 Информационная безопасность")
                            },
                            new KeyboardButton[]
                            {
                                new KeyboardButton("/fac 10.05.03 Информационная безопасность автоматизированных систем")
                            },
                        };
                        markup.OneTimeKeyboard = true;
                ...
                else if (e.Message.Text.Contains("/fac"))
                {
                    ...
                    else
                    {
                        ...
                        if (temp == "/fac 01.03.02 Прикладная математика и информатика") user.Facultet = User.Facultets.InfoMath;
                        else if (temp == "/fac 09.03.01.01 Веб-технологии") user.Facultet = User.Facultets.Web;
                        ...
                        else if (temp == "/fac 09.03.03.01 Корпоративные информационные системы") user.Facultet = User.Facultets.EnterpriseInfo;
                        else if (temp == "/fac 09.03.03.02 Большие и открытые данные") user.Facultet = User.Facultets.Data;
                        else if (temp == "/fac 10.03.01 Информационная безопасность") user.Facultet = User.Facultets.InfoSec;
                        else if (temp == "/fac 10.05.03 Информационная безопасность автоматизированных систем") user.Facultet = User.Facultets.InfoSecAuto;
                        else { bot.SendTextMessageAsync(e.Message.Chat.Id, "Пожалуйста, используйте клавиатуру"); return; }
                        var markup = new ReplyKeyboardMarkup();
                        markup.Keyboard = new KeyboardButton[][]
                        {
                ...
                else if (e.Message.Text.Contains("/fin"))
                {
                    ...
                    else
                    {
                        ...
                        bot.SendTextMessageAsync(e.Message.Chat.Id, "Вот и все. Бот запомнил информацию и может выдать результаты по команде /show. Если бот вас послал, то пройдите регистрацию еще раз по команде /start. Также вы можете включить оповещение об изменившемся месте командой /alarm on или /alarm off. Спасибо за использование\n(С) WinWins", replyMarkup: new ReplyKeyboardRemove()); return;
                    }
                }
                else if (e.Message.Text.Contains("/alarm"))
                {
                    User user = null;
                    foreach (User _user in users) if (_user.ChatId == e.Message.Chat.Id) user = _user;
                    if (user == null) { bot.SendTextMessageAsync(e.Message.Chat.Id, "Используйте команду /start", replyMarkup: new ReplyKeyboardRemove()); return; }
                    else
                    {
                        if (e.Message.Text.Split(' ').Length != 2)
                        {
                            bot.SendTextMessageAsync(e.Message.Chat.Id, "Команда /alarm использована неверно");
                            return;
                        }
                        string temp = e.Message.Text.Split(' ')[1];
                        if (temp == "on")
                        {
                            if (!user.bw.IsBusy) 
                            {
                                user.bw.RunWorkerAsync();
                                bot.SendTextMessageAsync(e.Message.Chat.Id, "Оповещения включены");
                            }
                            else bot.SendTextMessageAsync(e.Message.Chat.Id, "Пока обработчик оповещений все еще занят, пожалуйста попробуйте чуть позднее");
                        }
                        else if (temp == "off")
                        {
                            if (user.bw.IsBusy)
                            {
                                user.bw.CancelAsync();
                                bot.SendTextMessageAsync(e.Message.Chat.Id, "Оповещения отключены");
                            }
                            else bot.SendTextMessageAsync(e.Message.Chat.Id, "Пока обработчик оповещений все еще занят, пожалуйста попробуйте чуть позднее");
                        }
                        else bot.SendTextMessageAsync(e.Message.Chat.Id, "Команда /alarm использована неверно");
                    }
                }
                else
                {
                            ...
                            for (int i = 0; i < rows.Count; i++)
                            {
                                if (rows[i].Contains(snils))
                                {
                                    ...
                                    foreach (string listr in lis)
                                    {
                                        ...
                                        else if (listr.Contains("С СОГЛАСИЯМИ О ЗАЧИСЛЕНИИ НА ДРУГИЕ НАПРАВЛЕНИЯ"))
                                        {
                                            string temp1 = listr.Split(':')[1];
                                            for (int m = 1; m < temp1.Length; m++)
                                            {
                                                if (temp1[m] == '<') break;
                                                else agreeother += temp1[m];
                                            }
                                        }
                                        else if (listr.Contains("С СОГЛАСИЯМИ О ЗАЧИСЛЕНИИ"))
                                        {
                                            string temp1 = listr.Split(':')[1];
                                            for (int m = 4; m < temp1.Length; m++)
                                            {
                                                if (temp1[m] == '<') break;
                                                else agree += temp1[m];
                                            }
                                    ...
                                    string reply = "Ваше место в рейтинге: " + konkurs + "\n" +
                                        ...
                                        "Всего заявлений: " + all + "\n" +
                                        "С согласиями на зачисление: " + agree + "\n" +
                                        "С согласиями на другие направления: " + agreeother + "\n" +
                                    ...
                                    Console.WriteLine($"Facultet: {user.FacultetString}");
